...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  for (int i = 0; i < h; i++) {
    newBoard->board[i] = malloc(w * sizeof(*newBoard->board[i]));
    for (int j = 0; j < w; j++)
      newBoard->board[i][j] = UNKNOWN;
  }
  for (int i = 0; i < newBoard->totalMines; i++)
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if ((x + dx) < b->width && (x + dx) >= 0 && (y + dy) >= 0 && (y + dy) < b->height) {
      if (IS_MINE(b->board[x + dx][y + dy]))
        mines++;
    }
  }
  return mines;
